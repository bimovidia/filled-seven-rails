...
1. Be sure to **restart your development server** after a `bundle update filled-seven-rails` to get the latest version of filled-seven-rails.
1. **Make sure the icon still exists** [in FilledSeven](http://www.pixeden.com/icon-fonts/filled-7-icon-font-set) (sometimes icons are renamed between versions).
...
1. Make sure Rails isn't picking up a cached asset pipeline resource (`rm -rf tmp/cache`).
...
1. Please make sure that **no other gem dependencies in your project also include outdated versions** of the FilledSeven fonts.
1. If still having trouble, paste the results of `bundle list | grep  'rails\|filled\|sass\|less'` (full `bundle list` can sometimes be better)
